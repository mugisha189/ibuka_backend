...
export class MemberService {
    ...
    async getIbukaMembers(
        ...
        try {
            ...
            const memberDtos = MemberMapper.toIbukaMembersDtoList(members);
    ...
    async addMemberToFamily(familyId: string, memberDto: CreateMemberDto): Promise<ResponseDto<string>> {
        try {
            console.log(memberDto.profile_picture)
            const fileRes = await this.filesService.getFileById(memberDto.profile_picture, true) as any;
            if (fileRes && typeof fileRes === 'object' && 'url' in fileRes) {
                memberDto.profile_picture = fileRes.url;
            }
